...
def train_process(data_train, label_train, network, k_number, learning_rate=0.001, epoch_number=30, batch_size=32,
    ...
    for i in range(k_number):
        ...
        # 对每一份数据进行训练
        loss_train, loss_validation = train(network[i], dataset, data_validation, label_validation, learning_rate,
...
def train_final_network(network, data_train, label_train, learning_rate, epoch_number, weight_decay, batch_size,
                        file_name="net.pkl"):
    """
    :param network: 构造的神经网络
    ...
    """
    dataset = bc.TrainDataSet(data_train, label_train)
    loss_train, _ = train(network, dataset, None, None, learning_rate, epoch_number, weight_decay, batch_size)
    # 画出训练过程中的accuracy和loss变换曲线
