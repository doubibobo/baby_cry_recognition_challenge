...
if __name__ == '__main__':
    ...
    fe.write_data_to_csv_file(headers, file_label_indexes, "../data/csv_data/data_for_cnn_25ms_10ms.csv", "train", to_frame=True)

    # 建立测试集文件路径与标签的索引
    test_label_indexes = bf.get_filename("test")
    print(test_label_indexes)
    # 进行测试集的特征提取，并将其写入csv文件中
    fe.write_data_to_csv_file(headers, test_label_indexes, "../data/csv_data/test_for_cnn_25ms_10ms.csv", "test", to_frame=True)

    # 使用gpu进行训练
    gpu_available = gs.gpu_selector()

    # 构建卷积神经网络
    network = [net.CNNClassify(1, OUTPUT_SIZE) for i in range(K)]

    # 打印神经网络的结构
    summary(network[0], (1, TIME_STEP, INPUT_SIZE))
    ...
    # 将所有数据投入到最终的神经网络进行训练
    final_network = net.CNNClassify(1, OUTPUT_SIZE)
